...
const Topbar = ({ user, onLogout, notifications = [], onClearNotifications, onMarkAllRead, events = [], students = [], attendanceRecords = [] }) => {
  ...
  return (
    <div className="sticky top-0 z-10 border-b border-slate-800/80 bg-slate-950/80 px-4 py-4 backdrop-blur-xl md:px-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3 text-slate-200">
          <CalendarClock className="h-5 w-5 text-sky-300" />
          <span className="text-sm font-medium">{new Date().toLocaleDateString()}</span>
        </div>
        <form onSubmit={handleSearchSubmit} className="relative flex flex-1 items-center gap-3 rounded-full border border-slate-800 bg-slate-900/70 px-4 py-2 shadow-[0_24px_80px_rgba(15,23,42,0.24)]">
          ...
          />
          {keyword && (
            <div className="absolute left-0 right-0 top-full z-30 mt-3 overflow-hidden rounded-[24px] border border-slate-800 bg-slate-950 shadow-[0_24px_70px_rgba(2,6,23,0.5)]">
              {searchResults.length > 0 ? (
                searchResults.map((result) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-4 border-b border-slate-800 px-4 py-3 text-left text-sm transition last:border-b-0 hover:bg-slate-900"
                  >
                    <span>
                    ...
                    <span className="rounded-full bg-sky-500/10 px-3 py-1 text-xs font-semibold text-sky-300">{result.type}</span>
                  </button>
                ))
              ) : (
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-end">
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-800/80 bg-slate-900/85 px-4 py-2 text-sm text-slate-200 transition hover:bg-slate-800"
            >
              <Bell className="h-4 w-4" />
              Notifications
              {unreadCount > 0 && (
                <span className="ml-1 inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-rose-500 px-2 text-[11px] font-semibold text-white">
                  {unreadCount}
                </span>
              )}
            </button>
            {showNotifications && (
              <div className={`absolute right-0 z-20 mt-3 ${panelWidth} rounded-[28px] border border-slate-800/80 bg-slate-950/95 p-4 shadow-[0_30px_80px_rgba(15,23,42,0.4)]`}>
                <div className="mb-4 flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-700/80 bg-slate-900/90 p-2 text-slate-300 transition hover:bg-slate-800"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                  {notifications.length > 0 ? (
                    notificationsToShow.map((note) => (
                      <div
                        key={note.id}
                        className={`rounded-3xl border px-4 py-3 text-sm transition ${note.read ? 'border-slate-800/80 bg-slate-900/80 text-slate-300' : 'border-sky-500/90 bg-slate-900 text-slate-100 shadow-[0_8px_30px_rgba(56,189,248,0.12)]'}`}>
                        <div className="flex items-start justify-between gap-3">
                          <p className="font-medium leading-6">{note.message}</p>
                          {!note.read && <span className="rounded-full bg-sky-500 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white">New</span>}
                        </div>
                  ...
                  <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="rounded-full border border-slate-700/80 bg-slate-900/90 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-800"
                      >
                        Clear all
                      </button>
                    </div>
          ...
          <button
            ...
            className="hidden items-center gap-3 rounded-full border border-slate-800/80 bg-slate-900/80 px-4 py-2 text-left shadow-[0_16px_40px_rgba(15,23,42,0.18)] transition hover:bg-slate-800 md:flex"
          >
            {user?.profilePhoto ? (
          ...
          <button className="rounded-2xl bg-sky-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-400" onClick={onLogout}>
            Logout
          </button>
        </div>
      </div>
